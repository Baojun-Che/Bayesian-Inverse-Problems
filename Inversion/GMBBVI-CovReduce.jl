...
function update_ensemble!(gmgd::BBVIObj{FT, IT}, func_Phi::Function, dt_max::FT) where {FT<:AbstractFloat, IT<:Int} #从某一步到下一步的步骤
    ...
    for im = 1:N_modes 
        ...
        log_ratio, x_p_norm = zeros(N_ens), zeros(N_ens) 
        for i = 1:N_ens
            for imm = 1:N_modes
            ...
            x_p_norm[i] = norm(inv_sqrt_xx_cov[im]*(x_p[i,:]-x_mean[im,:]))^2
        end

        # E[logρ+Phi]
        log_ratio_mean = mean(log_ratio)
        x_p_norm_mean = mean(x_p_norm)

        alpha = (dot(log_ratio.-log_ratio_mean,x_p_norm.-x_p_norm_mean))/(norm(x_p_norm.-x_p_norm_mean)^2)

        bound = 50
        if alpha > bound  alpha=bound  end
        if alpha < -bound  alpha=-bound  end
        # if gmgd.iter%50==0   
        #     @show alpha, N_ens
        #     @show norm(x_p_norm.-x_p_norm_mean)^2/N_ens
        # end
        # alpha = 0
        ...
        log_ratio_m2 = 2*alpha*xx_cov[im,:,:] + mean( (x_p[i,:]-x_mean[im,:])*((x_p[i,:]-x_mean[im,:])'*(log_ratio[i]-log_ratio_mean-alpha*(x_p_norm[i]-x_p_norm_mean))) for i=1:N_ens)
